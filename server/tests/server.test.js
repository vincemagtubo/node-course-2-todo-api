...
describe('GET /todos/:todoID', () => {
    ...
    it('should not return todo doc created by motherfucking other', (done) => {
        request(app)
...
describe('DELETE /todos/:todoIDrmv', () => {
    ...
    it('should not remove a todo by motherfucking other user', (done) => {
        ...
        request(app)
            ...
            .end((err, res) => {
                if (err) {
                    ...
                    Todo.findById(hexID).then((_testrmv) => {
                        expect(_testrmv).toExist(); //toExist as it aint deleted for wrong user
                        done();
                    }).catch((err) => done(err));
                }
            })
...
describe('PATCH /todos/:todoIDupd', () => {
    ...
    it('should not update the todo if created by otther user', (done) => {
        var hexID = todos[2]._id.toHexString();
        var updText = 'Changed text, this is.';
            ...
            .end((err, res) => {
                if (err) {
                    ...
                    User.findById(users[1]._id).then((testUserLog) => {
                        expect(testUserLog.tokens.length).toBe(1);
                        done();
                    }).catch((err) => done());
                }
            })
